    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                nextVisitedCorner = set(visitedCorner)
                if nextPosition in self.corners \
                        and nextPosition not in visitedCorner:
                    nextVisitedCorner.add(nextPosition)
...
def cornersHeuristic(state, problem):
    ...
    remainingCorners = set(corners) - visitedCorners
    heuristic = 999999
    for corner in remainingCorners:
        totalDistance = util.manhattanDistance(position, corner)
        remainingCorners2 = set(remainingCorners)
        remainingCorners2.discard(corner)
        corner1 = corner
        while (len(remainingCorners2)):
            closestCorner = None
            closestCornerDistance = 999999
            for corner2 in remainingCorners2:
            ...
            remainingCorners2.discard(closestCorner)
...
def foodHeuristic(state, problem):
    ...
    for point in foodlist:
        ...
        lowCost = min(costArray)
    minI = costArray.index(lowCost)
    low = foodlist[minI]
    for point in foodlist:
        cost = abs(low[0] - point[0]) + abs(low[1] - point[1])
        newCostArray.append(cost)
        highCost = max(newCostArray)
    maxI = newCostArray.index(highCost)
    high = foodlist[maxI]
